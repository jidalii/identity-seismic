...
// src/chain.ts
var serializeSeismicTransaction = (transaction, signature) => {
  const {
    chainId,
    nonce,
    gasPrice,
...
var seismicDevnet2 = createSeismicDevnet({ node: 1 });
...
var remapSeismicParam = (param) => {
  ...
  if (ty.startsWith("tuple")) {
    const { components, ...rest } = param;
    const mappedComponents = components.map(remapSeismicParam);
    const shielded = isAnyTrue(mappedComponents.map(({ shielded: shielded2 }) => shielded2));
    return {
      shielded,
...
var gcm = /* @__PURE__ */ (() => generate(mode.GCM))();
...
class AesGcmCrypto {
  ...
  createNonce(num2) {
    return bytesToHex(this.numberToNonce(num2));
  }
  async encrypt(plaintext, nonce) {
...
var sharedSecretPoint = ({
  ...
  return secp256k1.getSharedSecret(privateKeyHex, networkPublicKey, false).slice(1);
};
var sharedKeyFromPoint = (sharedSecret) => {
  const version4 = sharedSecret[63] & 1 | 2;
  const finalSecret = sha256.create().update(new Uint8Array([version4])).update(sharedSecret.slice(0, 32)).digest();
...
var deriveAesKey = (sharedSecret) => {
  const derivedKey = hkdf(sha256, hexToBytes(`0x${sharedSecret}`), new Uint8Array(0), new TextEncoder().encode("aes-gcm key"), 32);
...
class SignedCallError extends BaseError2 {
  constructor({
    docsPath: docsPath6,
    metaMessages,
    reason
...
var seismicTxTypedData = (tx, signedCall = false) => {
  if (!tx.chainId) {
    throw new Error("Seismic transactions require chainId argument");
  }
...
var signSeismicTxTypedData = async (client, tx, signedCall = false) => {
  const typedData = seismicTxTypedData(tx, signedCall);
  const encodedSignature = await signTypedData(client, typedData);
  const { r, s, yParity } = parseSignature2(encodedSignature);
  return {
    typedData,
    signature: {
      r,
...
var doSignedCall = async (client, seismicTx, { block }) => {
  if (client.account?.type === "json-rpc") {
    const { typedData, signature } = await signSeismicTxTypedData(client, seismicTx, true);
  ...
  const serializedTransaction = await client.account.signTransaction(seismicTx, {
...
async function signedReadContract(client, parameters) {
  const {
    abi: abi2,
    functionName,
    args = [],
  ...
  const selector = toFunctionSelector(formatAbiItem2(seismicAbi));
...
class AccountNotFoundError2 extends BaseError2 {
  constructor({ docsPath: docsPath6 } = {}) {
    super([
      "Could not find an Account to execute with this Action.",
      "Please provide an Account with the `account` argument on the Action, or by supplying an `account` to the Client."
...
class AccountTypeNotSupportedError2 extends BaseError2 {
  constructor({
    docsPath: docsPath6,
    metaMessages,
    type
...
async function sendShieldedTransaction(client, parameters) {
  ...
  try {
    ...
    if (account === null || account?.type === "local" || account?.type === "json-rpc") {
      let chainId;
      if (chain !== null) {
        chainId = await getAction(client, getChainId, "getChainId")({});
        assertCurrentChain({
      ...
      if (account?.type === "json-rpc") {
        const { typedData, signature } = await signSeismicTxTypedData(client, txRequest);
        const action = getAction(client, sendRawTransaction, "sendRawTransaction");
        return await action({
          serializedTransaction: { data: typedData, signature }
        });
      } else {
        const serializedTransaction = await account.signTransaction(txRequest, { serializer: serializeSeismicTransaction });
...
function getShieldedContract({
  ...
  const viemContract = getContract({ abi: abi2, address, client });
  const walletClient = (() => {
    if (!client)
      return;
    if ("wallet" in client)
    ...
    get(_, functionName) {
      return (...parameters) => {
        if (!walletClient?.account) {
          ...
          throw new Error("Wallet must have an account with address to perform signed reads");
        }
        const params = getFunctionParameters2(parameters);
        const { args } = params;
        const {
...
// src/actions/encryption.ts
var encryptionActions = (encryption, encryptionPublicKey) => {
  return {
...
// src/actions/public.ts
var shieldedPublicActions = (client) => ({
  ...
  publicRequest: async (_args) => client.request(_args)
});
...
var shieldedWalletActions = (client) => {
  return {
    ...
    readContract: (args) => signedReadContract(client, args),
    treadContract: (args) => readContract(client, args),
    twriteContract: (args) => writeContract(client, args),
    signedCall: (args) => signedCall(client, args),
    sendShieldedTransaction: (args) => sendShieldedTransaction(client, args)
  };
};
...
var createShieldedPublicClient = (parameters) => {
  const viemPublicClient = createPublicClient(parameters);
  return viemPublicClient.extend(shieldedPublicActions);
};
var getSeismicClients = async ({
  ...
  const pubClient = publicClient ?? await createShieldedPublicClient({
    chain,
    transport
  });
  ...
  }).extend(walletActions).extend(() => publicActions(pubClient)).extend(() => encryptionActions(aesKey, encryptionPublicKey)).extend(() => shieldedPublicActions(pubClient)).extend(shieldedWalletActions);
  return {
    public: pubClient,
    wallet
  };
};
var createShieldedWalletClient = async ({
  chain,
  transport,
  account,
  encryptionSk,
